...
The Demographic and Health Surveys program (DHS) organizes and conducts household surveys in low- and middle-income countries. DHS first aggregates the collected data into the GPS coordinates of each household cluster center and then randomly diplaces their GPS coordinates in order make the participants and their locations untracable. We call this small displacements "jittering". The true coordinates are jittered with respect to a known random displacement algorithm which applies different maximum jittering distances for urban and rural areas. The urban cluster centers are jittered up to two kilometers, while the rural ones are jittered up to 5 kilometers (with the 1% of them being jittered up to 10 kilometers)[@DHSspatial07]. 

<!-- DHS applies two levels of measures to protect privacy of the survey participants. First, the individual level data is aggregated into the household cluster centers, so that the individual responses become indistinguishable. On top of this, the true coordinates of the cluster centers are geographically masked by randomly displacing (jittering) them. The true coordinates are displaced with respect to a known jittering algorithm which applies different maximum displacement distances for urban and rural areas. The urban cluster centers are jittered up to two kilometers, while the rural ones are jittered up to 5 kilometers (with the 1% of them being jittered up to 10 kilometers)[@DHSspatial07].  -->

The random displacement leads the observed coordinates to contain positional error. We discovered that such error results in attenuated (towards zero) model parameter estimates and poor predictive performance, unless it is addressed [@altay2022accounting], [@altay2022covariates]. This package aims to provide fast and useful tools that allow doing inference and prediction while adjusting/accounting for the positional error in the GPS coordinates of the survey cluster centers.
