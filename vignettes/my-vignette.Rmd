...
The random displacement leads the DHS coordinates to contain positional error. We recently discovered that the unaddressed positional error results in the model parameter estimates that are attenuated towards zero together with a poorer predictive performance [@altay2022accounting], [@altay2022covariates]. GeoAdjust package is constructed in order to provide useful computational tools which accounts for positional uncertainty in the GPS coordinates of the observed data. The package follows the methods that are developed and discussed in detail in [@altay2022accounting] and [@altay2022covariates].
...
## THE MODEL
Assume that we want to estimate the risk of completing secondary education among 20-39 years old women throughout Nigeria. We call the locations that are indicated by the GPS coordinates of the centroids of each household cluster as the "true locations", whereas the published GPS coordinates of the cluster centers are refferred to as the "observed locations". These locations contain positional error due to being jittered, and in order to avoid the impact of its adverse effects on inference, we need to adjust for this error in our analysis. 
...
In this observation model, both $y_c$ and $\boldsymbol{s}_c$ are already available within the DHS data set and accordingly, treated as the observed quantities. The unobserved true locations $s_c^*$ on the oether hand, are inaccesible and they are treated as random quantities and assigned a uniform prior $s_c^*\sim \mathcal{U}(\mathcal{D})$. This implies that we treat all locations $\boldsymbol{s}_c^*$ within the maximum jittering distance from $\boldsymbol{s}_c$ as equally likely \emph{a priori}, meaning that the true location can be located anywhere within the corresponding jittering radius with equal probability.

## DATA PREPARATION
The survey data consists of both individual level and cluster level information. The individual level data is a Stata file that mainly contains the responses of the participants to the survey. The file has a variable indicating the corresponding cluster ID of each data row as well. The cluster level data is a shape file. It contains cluster identification information such as their jittered latitudes and longitudes (in degrees), cluster ID, corresponding strata ("R" for rural and "U" for urban) and the name of the administrative area that they are located within. Since the cluster ID is a common variable in both files, it is useful while merging them into a single data frame. 

It is not possible to share DHS data publicly, therefore the real survey data will not be processed through this vignette. Instead, an example data set that is generated specifically in order to mimic the DHS data structure will be used. On the other hand, it might still be potentially useful to show how to extract individual and cluster level data and merge them into a single data frame. It might save time for the users of the package to follow through the steps quickly and obtain the variables of interest. We will show how to do it based on the Nigeria 2018 DHS survey and please note that the data file names and variable names might differ in other surveys.

Lastly, two more shape files are needed. One of them is the shape file that contains the national (admin0) level borders of the country of interest. The second one should contain the borders that correspond to the administrative level areas with respect to which the true cluster centers are jittered. Shape files of various administrative levels for different countries are available at the  website of "the Database of Global Administrative Areas (GADM)" (https://gadm.org/data.html). Once downloaded, the files can be read into R as "SpatialPolygonsDataFrame" objects.
...
# Cluster level data (clusterID, cluster coordinates, urbanicity, etc.)
...
myData = data.frame(clusterIdx = educationData$v001,         # ID numbers of clusters
                    householdIdx = educationData$v002,       # ID numers of households
                    stratum = educationData$v023,            # stratum of the cluster (urban or rural)
...
### Fine tuning:
```{r eval = FALSE}
# jitterin ia done by respecting admin2 borders in Nigeria. 
